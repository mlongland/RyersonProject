#### This is to take the scrape of RateBeer.com from Import.io and convert it
#### into csv files to be processed in Spark, R, etc.
			...
			for x in range(0,len(parsedReview)):
				...
				if isinstance(item, basestring):
					if x!=len(parsedReview)-1:
						f.write("\""+item.replace("\"","\'").encode("utf-8")+"\"\t")
					else:
						f.write("\""+item.replace("\"","\'").encode("utf-8")+"\"")
				else:
					f.write("%.3f" % item)
